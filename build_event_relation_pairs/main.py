...
def select_event_relation_pairs(output_dir: str, threshold: int):
    ...
    for (event_idx, event), rel_scores in records.items():
        ...
        results.append(
            {"event_idx": event_idx, "event": event, "relations": good_relations}
        )
...
def main():
    ...
    samples = get_unprocessed_samples(
        data_file_path=config["atomic_file_path"],
        output_file_path=config["rate_file_path"],
        id_field="event_idx",
    )
    ...
    if config["select_event_relation_pairs"]:
        ...
        select_event_relation_pairs(
            output_dir=config["output_dir"], threshold=config["threshold"]
        )
